...
class MatcherTests extends FlatSpec {
  ...
  "Matcher that is not passed a root location" should "use the current location" in {
    val matcher = new Matcher("filter")

    assert(matcher.rootLocation == new File(".").getCanonicalPath)
  }

  "Matcher with sub folder checking matching a root folder location with two subtree files matching " should
    "return a list with those filenames" in {
