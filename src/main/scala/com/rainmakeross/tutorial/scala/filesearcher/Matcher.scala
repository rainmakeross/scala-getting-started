...
class Matcher(val filter:String, val rootLocation: String = new File(".").getCanonicalPath, checkSubFolder: Boolean = false, contentFilter: Option[String] = None) {
  ...
  def execute() = {
    ...
    val contentFilteredFiles = contentFilter match {
      case Some(dataFilter) => matchedFiles
        .map(iOObject =>(iOObject.name, Some(FilterChecker(dataFilter).findMatchedContentCount(iOObject.file))))
        .filter(matchTuple=>matchTuple._2.getOrElse(0)>0)
      case None => matchedFiles map (iOObject => (iOObject.name, None))
    }
